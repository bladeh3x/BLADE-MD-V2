...
smd({
    ...
    async({reply, chat ,isGroup, sender,error}) => {
      try{
        ...
        const daily  = await eco.daily(sender, "BLADE-BOT", 500); //give 500 for daily, can be changed
        if (daily.cd) {
...
smd({
    ...
    async(message) => {
      try{
       ...
       const balance  = await eco.balance(users, "BLADE-BOT")
       await eco.deduct(users, "BLADE-BOT", balance.wallet);
...
async(message,match) => {
  try{
   ...
   const balance  = await eco.balance(user, "BLADE-BOT")
   switch (value) {
       case '1000':
       case '1':
       if (k > balance.wallet ) return message.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
         const deduct1 = await eco.deduct(user, "BLADE-BOT", 100);
         const add1 = eco.giveCapacity(user,"BLADE-BOT", 1000);
       ...
       if (k < balance.wallet) return message.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
         const deduct2 = await eco.deduct(user,"BLADE-BOT", 1000);
         const add2 = eco.giveCapacity(user, "BLADE-BOT", 100000);
       ...
       if (k < balance.wallet) return message.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
          const deduct3 = await eco.deduct(user, "BLADE-BOT", 10000);
          const add3 = eco.giveCapacity(user, "BLADE-BOT", 10000000);
