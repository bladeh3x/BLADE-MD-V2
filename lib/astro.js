...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "BLADE-MD-V2"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "BLADE-MD-V2") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "BLADE-MD-V2") => {
...
app.listen(randomPort, () =>
  console.log(
    "BLADE-MD-V2 Server listening on http://localhost:" + randomPort + "/ "
  )
);
